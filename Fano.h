...
public:
    ...
    List<char> getSymb (){
        return symbolsInString.get_keys();
    }
    void fanoCoding();
    void fanoDecoding();
    void printInfo();
    string getCoded(){ return encodedStr; }
    string getDecoded(){ return decodedStr; }
private:
    string input;
    string encodedStr = "";
    string decodedStr = "";
...
void fanoCreateCode(int size, MapNode<char, int> *arrayOfSymbols[])
{
	if (size > 1) {
		...
		for (int i = 0; i < size; i++) {
			frequenciesSum += arrayOfSymbols[i]->value;
		}
...
void Fano::fanoCoding() {
	...
	string codedString;
	for (char o : input) {
		codedString += symbolsInString.find(o)->code;
	}
